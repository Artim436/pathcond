...
def test_original_model_is_not_modified(sizes=(32, 16)):
    ...
    _ = apply_neuron_rescaling_mlp(m, layer_idx=1, neuron_idx=2, lamda=1.3)
...
def test_apply_rescaling(sizes=(4, 4)):
    torch.manual_seed(0)
    m = MNISTMLP(d_hidden1=sizes[0], d_hidden2=sizes[1], p_drop=0.0).eval()
    linear_indices = [i for i, layer in enumerate(m.model) if isinstance(layer, nn.Linear)]
    n_hidden_neurons = sum(m.model[i].out_features for i in linear_indices[:-1])
